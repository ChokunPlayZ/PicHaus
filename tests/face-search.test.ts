...
describe('matchReferenceFaces', () => {
    ...
    it('keeps legitimate repeated appearances of the same person in different spots', () => {
        // Same embedding but NON-overlapping boxes (before/after split, mirror,
        // collage, double exposure): still ONE person → one group.
        const same = [1, 0, 0]
        const candidates = [candidate('alice-photo', same)]
    ...
    it('keeps different people separate even when boxes do not overlap', () => {
        // Two genuinely different faces (different embeddings) → two groups.
        const alice = [1, 0, 0]
        const bob = [0, 1, 0]
        const candidates = [candidate('alice-photo', alice), candidate('bob-photo', bob)]
        ...
        const groups = matchReferenceFaces(
            [
                referenceFace(alice, { x1: 0.05, y1: 0.05, x2: 0.35, y2: 0.45 }),
                referenceFace(bob, { x1: 0.6, y1: 0.55, x2: 0.9, y2: 0.95 }),
            ],
            candidates,
            { threshold: 0.5 },
        )
    ...
    it('keeps lookalikes separate when embeddings differ enough', () => {
        // Twins/lookalikes with near-but-not-identical embeddings: still two
        // people, since identity is decided by the embedding similarity alone.
        const twinA = [1, 0, 0]
        const twinB = [0.3, 0.95, 0]
