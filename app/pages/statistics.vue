    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div v-else class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-[var(--glass-bg)] backdrop-blur-xl rounded-2xl p-6 border border-[var(--glass-border-light)] shadow-xl flex items-center gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
             <div v-for="(type, key) in { aperture: 'Aperture', iso: 'ISO', shutterSpeed: 'Shutter', focalLength: 'Focal Length' }" :key="key" 
        ...
        <!-- Timeline -->
        <div class="bg-[var(--glass-bg)] backdrop-blur-xl rounded-2xl p-8 border border-[var(--glass-border-light)] shadow-xl">
             <h2 class="text-2xl font-bold text-[var(--text-primary)] mb-6">Activity Timeline</h2>
             <div class="h-64 flex items-end gap-2 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                  <div v-for="(month, idx) in stats?.timeline" :key="idx" class="flex flex-col items-center gap-2 group flex-shrink-0" style="width: 40px">
                      <div class="w-full bg-[var(--glass-bg-strong)] rounded-t-sm relative transition-all duration-300 group-hover:bg-[var(--btn-primary-start)]" 
                           :style="{ height: `${calculatePercentage(month.count, Math.max(...stats.timeline.map((t: any) => t.count)))}%`, minHeight: '4px' }">
                           <div class="absolute -top-8 left-1/2 -translate-x-1/2 bg-black/80 px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 whitespace-nowrap transition-opacity pointer-events-none border border-white/10 z-20">
                               {{ month.count }} photos
                           </div>
                      </div>
                      <div class="text-[10px] text-[var(--text-muted)] -rotate-45 origin-top-left translate-y-2 whitespace-nowrap">{{ month.date }}</div>
                  </div>
             </div>
        </div>
      </div>
